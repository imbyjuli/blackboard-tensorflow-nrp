...
def read_nrp_data (path,mode_name, black_white = False,nm_start =0 , nm_end = 10000, num_classes = 4): 
  with open(path +  'labels.csv','r') as csvfile:
      reader = csv.reader(csvfile, delimiter=' ', quotechar='|')
      csv_list =   [','.join(row).split(',') for row in reader][1:]
  csv_list = csv_list[nm_start: nm_end]
  #reading in images 
  if black_white: 
    data = [cv2.imread( (path + name), cv2.IMREAD_GRAYSCALE).flatten() for name,_,_,_,_,_,_,_,_ in csv_list]
  else: 
    data = [cv2.imread( path + name).flatten() for name,_,_,_,_,_,_,_,_ in csv_list]


  labels_temp = [[tl, tr, bl, br] for _,tl, tr, bl, br,_,_,_,_  in  csv_list]
...
def read_nrp_data_predict(path, black_white = False,nm_start =0 , nm_end = 10000, num_classes = 4): 
  with open(path + 'labels.csv','r') as csvfile:
      reader = csv.reader(csvfile, delimiter=' ', quotechar='|')
      csv_list =   [','.join(row).split(',') for row in reader][1:]
  ...
  csv_list = [row for row in csv_list if any( location != "empty" for location in row[1:5])]
  ...
  labels_temp = [[("top_left",tl), ("top_right",tr),("bottom_left", bl), ("bottom_right",br)] for _,tl, tr, bl, br,_,_,_,_  in  csv_list]
